 ...
 *
 */
public class RSFaceFrontTrackTask implements Runnable {
	...
	public RSFaceFrontTrackTask(VideoPanel vp) {
		videoPanel = vp;
	}
	...
	private PITData createFaceData(BufferedImage frame, PXCMFaceData.DetectionData detection) {
		...
		if (ret) {
			...
			if (Config.getInstance().getFrontFaceTrackMode() == Config.FACE_TRACK_IR) {
	...
	public void doTracking() throws Exception {
		...
		else if (Config.getInstance().getFrontFaceTrackMode() == Config.FACE_TRACK_COLOR_DEPTH)
			faceConfig.SetTrackingMode(PXCMFaceConfiguration.TrackingModeType.FACE_MODE_COLOR_PLUS_DEPTH);
		else if (Config.getInstance().getFrontFaceTrackMode() == Config.FACE_TRACK_COLOR)
			faceConfig.SetTrackingMode(PXCMFaceConfiguration.TrackingModeType.FACE_MODE_COLOR);

		// PXCMFaceConfiguration.RecognitionConfiguration
		// rcfg=faceConfig.QueryRecognition();
		// rcfg.Enable();
		...
		while (startCapture) {
			...
			try {
				...
				for (SortedFace sf : sortFaces) {
					...
					// PXCMFaceData.RecognitionData
					// rdata=face.QueryRecognition();
					// int regRet = rdata.RegisterUser();
					// log.debug("frdata.RegisterUser=="+regRet);
					// int uid=rdata.QueryUserID();
					// log.debug("rdata.QueryUserID=="+uid);
