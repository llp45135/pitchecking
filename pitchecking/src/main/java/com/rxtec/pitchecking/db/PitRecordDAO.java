...
public class PitRecordDAO {
	...
	public void save(PitRecord rec) {
		...
		if (oldRec == null) {
			...
			Query query = new Query(Criteria.where("idNo").is(rec.getIdNo())
					.and("ticket.trainCode").is(rec.getTicket().getTrainCode())
					.and("ticket.trainDate").is(rec.getTicket().getTrainDate()));
	...
	public PitRecord queryRecord(String idNo, String trainDate, String trainCode) {
		Query query = new Query(Criteria.where("idNo").is(idNo)
				.and("ticket.trainCode").is(trainCode)
